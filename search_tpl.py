# -*- coding: utf-8 -*-
"""Test: templateSearch + templateDetail dari browser context."""
...
def main():
    ...
    # cek auth dengan query sederhana
    print("[4] Test latestTemplates (cek auth)...")
    r = gql(p, """
    query latestTemplates {
      templates(first: 3) {
    ...
    print("[5] templateSearch 'Hermes'...")
    r = gql(p, """
    query templateSearch($query: String!, $first: Int) {
      templateSearch(query: $query, first: $first) {
        edges { node { id code name } }
      }
    }
    """, {"query": "Hermes", "first": 5})
    print(f"    {r}")
