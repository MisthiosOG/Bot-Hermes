# -*- coding: utf-8 -*-
"""Test templateDetail dengan berbagai format code, lalu deploy."""
...
def main():
    ...
    # coba templateDetail berbagai format code
    codes = [
        "https://github.com/MisthiosOG/Hermes-Gateway",
        "github.com/MisthiosOG/Hermes-Gateway",
        "MisthiosOG/Hermes-Gateway",
        "hermes-gateway",
        "hermes-panel",
    ]
    for code in codes:
        ...
        """, {"code": code})
        has_data = '"data"' in r and '"template"' in r and 'null' not in r.split('"template"')[1][:50]
        print(f"[4] code='{code}': {'FOUND' if has_data else 'miss'}")
        if has_data:
            print(f"    {r[:600]}")
            break
