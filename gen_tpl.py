# -*- coding: utf-8 -*-
"""Test templateGenerate + templateDeployV2 dengan berbagai input."""
...
def gql(p, query, variables=None):
    ...
    return run_async_store(p, """
        ...
        window.__r = 'status=' + res.status + ' body=' + txt.substring(0, 1500);
    """)
...
def main():
    ...
    # coba templateGenerate dengan input tebakan
    print("[4] templateGenerate coba 1: repoUrl...")
    r = gql(p, """
    mutation templateGenerate($input: TemplateGenerateInput!) {
      templateGenerate(input: $input) { id code }
    }
    """, {"input": {"repoUrl": "https://github.com/MisthiosOG/Hermes-Gateway"}})
    print(f"    {r}")

    print("[5] templateGenerate coba 2: githubUrl...")
    r = gql(p, """
    mutation templateGenerate($input: TemplateGenerateInput!) {
      templateGenerate(input: $input) { id code }
    }
    """, {"input": {"githubUrl": "https://github.com/MisthiosOG/Hermes-Gateway"}})
    print(f"    {r}")

    print("[6] templateGenerate coba 3: repository...")
    r = gql(p, """
    mutation templateGenerate($input: TemplateGenerateInput!) {
      templateGenerate(input: $input) { id code }
    }
    """, {"input": {"repository": "https://github.com/MisthiosOG/Hermes-Gateway"}})
    print(f"    {r}")
