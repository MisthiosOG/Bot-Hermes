# -*- coding: utf-8 -*-
"""Test /new/github UI setelah login."""
...
def main():
    ...
    # coba /new/github
    p.get("https://railway.com/new/github")
    time.sleep(8)
    print(f"[4] /new/github URL: {p.url}")
    txt = p.run_js("return document.body.innerText.substring(0, 600)")
    print(f"    Body: {txt}")
    p.get_screenshot(path="new_github.png")

    # coba /new (biasanya ada pilihan Deploy GitHub Repo / Empty / Database)
    p.get("https://railway.com/new")
    time.sleep(8)
    print(f"[5] /new URL: {p.url}")
    txt2 = p.run_js("return document.body.innerText.substring(0, 800)")
    print(f"    Body: {txt2}")
    p.get_screenshot(path="new_page.png")
...
})).filter(x => x.t).slice(0, 30);
""")
    print(f"    Buttons/Links: {btns}")
