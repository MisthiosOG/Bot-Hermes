# -*- coding: utf-8 -*-
"""Introspect Railway GraphQL schema + login."""
...
def gql(p, query, variables=None):
    """Panggil GraphQL dari browser context."""
    if variables is None:
        variables = {}
    p.run_js(f"window.__gql_vars = {json.dumps(variables)};")
...
def main():
    ...
    # Introspect schema buat TemplateGenerateInput
    print("[4] Introspect schema...")
    r = gql(p, """
    query {
      __type(name: "TemplateGenerateInput") {
        name
        inputFields { name type { name kind ofType { name kind } } }
      }
      __type2: __type(name: "TemplateDeployV2Input") {
        name
        inputFields { name type { name kind ofType { name kind } } }
      }
    }
