# -*- coding: utf-8 -*-
"""Test: login + deploy template page UI."""
...
def main():
    ...
    # Cek dashboard udah login
    p.get("https://railway.com/dashboard")
    time.sleep(6)
    print(f"[6] Dashboard URL: {p.url}")

    # Buka halaman deploy template
    print("[7] Buka deploy template page...")
    p.get("https://railway.com/new/template?template=https://github.com/MisthiosOG/Hermes-Gateway")
    time.sleep(8)
    print(f"    URL: {p.url}")
    txt = p.run_js("return document.body.innerText.substring(0, 1000)")
    print(f"    Body: {txt}")

    p.get_screenshot(path="deploy_page_loggedin.png")

    # Cek input fields & buttons
    inputs = p.run_js("""
return Array.from(document.querySelectorAll('input, textarea, select')).map(el => ({
    ph: (el.placeholder || '').substring(0, 50),
...
""")
    ...
    btns = p.run_js("""
return Array.from(document.querySelectorAll('button')).map(b => (b.textContent || '').trim().substring(0, 60)).filter(x => x);
""")
