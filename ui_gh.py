# -*- coding: utf-8 -*-
"""Test UI /new -> GitHub Repository -> input URL repo publik."""
...
def main():
    ...
    # buka /new
    p.get("https://railway.com/new")
    time.sleep(8)
    print(f"[4] /new URL: {p.url}")
    txt = p.run_js("return document.body.innerText.substring(0, 400)")
    print(f"    Body: {txt}")

    # cari tombol/link GitHub Repository
    gh_link = p.run_js("""
const els = Array.from(document.querySelectorAll('a, button'));
const hit = els.find(el => (el.textContent || '').trim().includes('GitHub Repository'));
return hit ? JSON.stringify({ tag: hit.tagName, href: hit.href || '', text: (hit.textContent||'').trim().substring(0,50) }) : 'NOT_FOUND';
""")
    print(f"    GitHub Repository element: {gh_link}")

    # klik kalau ada
    if 'href' in gh_link:
        import json as _j
        info = _j.loads(gh_link)
        if info.get('href'):
            p.get(info['href'])
            time.sleep(8)
            print(f"    Navigated to: {p.url}")
            txt2 = p.run_js("return document.body.innerText.substring(0, 500)")
            print(f"    Body: {txt2}")
            p.get_screenshot(path="github_repo_page.png")
